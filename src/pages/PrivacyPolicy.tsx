...
interface PrivacyPolicyHeader { title: string; subtitle: string }
interface PrivacyPolicyContent { heading: string; body: string }

const PrivacyPolicy = () => {
  const content = getPageContent('privacy-policy');
  const header = content?.header as PrivacyPolicyHeader | undefined;
  const policyContent = content?.content as PrivacyPolicyContent[] | undefined;
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto">
            <div className="text-lg text-brand-black/80 leading-relaxed space-y-8">
              {policyContent?.map((section: PrivacyPolicyContent, i: number) => (
                <div key={i}>
